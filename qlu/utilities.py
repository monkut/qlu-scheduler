...
from typing import Callable, Generator, Hashable, Iterable
...
class AssigneeChooser:
    ...
    def __init__(
        self, assignees: Iterable[Hashable], distribution: Callable = DEFAULT_DISTRIBUTION_CALLABLE, dargs: tuple = None, dkwargs: dict = None
    ):
        """
        ...
        if distribution is uniform and not dargs:
            if not dkwargs:
                dkwargs = {"low": 0, "high": len(assignees)}  # inclusive
...
class PhantomUserAssignmentManager:
    ...
    def assign(
        self, tasks: Iterable[QluTask], distribution_func: Callable = DEFAULT_DISTRIBUTION_CALLABLE, dargs: tuple = None, dkwargs: dict = None
